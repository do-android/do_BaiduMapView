		...
		if (mRouteLine.getAllStep() != null && mRouteLine.getAllStep().size() > 0) {
			...
			for (DrivingRouteLine.DrivingStep step : mRouteLine.getAllStep()) {
				...
				if (step.getEntrance() != null) {
					overlayOptionses.add((new MarkerOptions()).position(step.getEntrance().getLocation()).anchor(0.5f, 0.5f).zIndex(10).rotate((360 - step.getDirection())).extraInfo(b)
...
//            }
			...
			PolylineOptions option = new PolylineOptions().points(points).textureIndex(traffics).width(7).dottedLine(isDotLine).focus(true)
